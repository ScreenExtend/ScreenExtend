...
- **System audio streaming (Windows & macOS).** Optionally stream the host's system audio to a device alongside the video, encoded as low-delay Opus and delivered over the same DTLS transport with a custom client-side jitter buffer (bypassing the browser's NetEQ, matching the video path). Capture is low-latency and native per platform: **WASAPI loopback** on Windows, and on macOS a runtime-tiered **Core Audio Process Tap** (macOS 14.2+, audio-only — no screen-recording indicator) falling back to **ScreenCaptureKit audio** (macOS 13.0+). On **macOS 10.15–12.x**, which has no native system-audio API at all, ScreenExtend installs its own signed **"ScreenExtend Audio"** virtual output device (a Core Audio HAL plug-in built on libASPL) via a one-time admin prompt, streams the captured mix over shared memory, and plays it back to your real speakers itself — keeping the macOS volume keys working, and restoring your previous output device when streaming stops. Off by default; enable it per device (Devices screen, or `ScreenExtend devices set <ip> --audio on`). **Privacy:** this captures *everything the host is playing* — including other apps, calls, and notifications — for that device, and on 10.15–12.x it also makes ScreenExtend your Mac's output device while streaming; both are why it is opt-in per device and defaults to off. Hosts below macOS 10.15 or on Linux show the toggle disabled with an explanatory tooltip.
