//! Unit tests for the legacy virtual-device backend (PRD-macos-legacy-audio.md §11). These cover
//! the parts that are pure logic and therefore verifiable without a live driver or Core Audio: the
//! shm overwrite/wraparound math + layout ABI, the routing state machine (incl. crash recovery),
//! and the volume→gain mapping and ramp. The Core Audio glue (IOProcs, HAL listeners) is exercised
//! only at runtime on a real 10.15–12.x host.

mod live;
mod ring;
mod routing;

// ── volume→gain mapping + smoothing (playthrough / volume_proxy) ────────────────────────────────
mod gain {
    ...
    fn ramp_moves_toward_target_without_overshoot() {
        // From 0 toward 1: monotonic, never exceeds target, converges.
        let mut g = 0.0f32;
        let mut prev = -1.0f32;
        for _ in 0..2000 {
            g = ramp(g, 1.0);
    ...
    fn drift_trim_pulls_ring_toward_setpoint() {
        ...
        // Ring too full → trim goes positive (consume faster to drain).
        let up = drift_trim(0.0, setpoint + 8000, setpoint);
        assert!(up > 0.0, "over-full ring should raise the trim, got {up}");
        // Ring too empty → trim goes negative (consume slower to refill).
    ...
    fn drift_trim_is_bounded() {
        // Even a wildly full ring can't push the trim past ±0.3 % (no audible pitch wobble).
        let mut t = 0.0;
