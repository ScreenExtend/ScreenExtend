//! Routing state machine incl. crash recovery (PRD-macos-legacy-audio.md §8.3, §11), exercised with
//! an in-memory fake default-device port so no Core Audio is involved.
...
/// Fake port: an in-memory "current default device UID" plus a fallback real-output UID.
struct FakePort {
...
fn activate_saves_previous_and_takes_over() {
    ...
    // We are now the default; the saved device is the previous one, persisted with changed=true.
...
fn restore_puts_the_saved_device_back_and_clears_flag() {
    ...
    // Persisted flag cleared; not active anymore.
...
fn activate_never_saves_ourselves_as_the_device_to_restore() {
    let path = temp_state_path();
    // Edge case: we are somehow already the default (e.g. a prior crash). The port offers a real
    // output device as fallback, and that — not ourselves — must be recorded to restore to.
...
fn user_switching_output_is_respected_not_fought() {
    ...
    // The user picks a different device out from under us.
    ...
    // We stopped asserting and cleared the flag; we do NOT re-set ourselves as default.
    assert!(!router.is_active());
    assert!(!RoutingState::load(&path).changed);
    let _ = std::fs::remove_file(&path);
}
...
fn default_change_back_to_us_is_ignored() {
    ...
    let mut router = Router::new(port, OURS.to_string(), path.clone());
    router.activate(1).unwrap();
    // A spurious notification while we're still the default → no-op.
...
fn crash_recovery_restores_when_we_died_holding_the_default() {
    let path = temp_state_path();
    // Simulate a crash: persisted state says we changed it, and the current default is still us.
    RoutingState {
...
fn crash_recovery_no_op_when_default_is_not_us() {
    let path = temp_state_path();
    // We flagged changed, but by launch the default is already something else (user fixed it).
    RoutingState {
    ...
    let recovered = recover(&port, OURS, &path);
    assert!(!recovered);
    // We must not stomp the user's current choice.
