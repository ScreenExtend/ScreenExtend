//! shm ring: overwrite/wraparound math (`clamp_read`), the shared-memory layout ABI, and the
//! generation-counter resync rule (PRD-macos-legacy-audio.md §11).

use crate::macos_utils::audio::legacy::shm_reader::{clamp_read, layout};

#[test]
fn no_lap_when_reader_keeps_up() {
    // Writer is 100 samples ahead of a 1024-capacity ring: nothing lost.
...
fn lap_skips_to_one_ring_behind_head() {
    // Writer got 1500 ahead of a 1024 ring: 476 samples are unrecoverable.
...
fn overwrite_wraparound_stays_consistent_over_many_writes() {
    // Simulate a slow reader against an always-advancing writer and confirm the reader never reads
    // a stale (pre-lap) region: after clamping, avail is always ≤ capacity and read_pos stays
    // within one ring of the head.
    let cap = 8u64;
    let mut read_pos = 0u64;
    let mut write_pos = 0u64;
    for step in 0..1000u64 {
        // Writer advances a variable amount each round.
...
fn generation_change_means_resync_to_head() {
    // The reader's rule on a generation bump is read_pos = write_pos (drop the backlog). Model it:
...
fn layout_matches_cpp_abi_contract() {
    // These MUST match macos/ScreenExtendAudio/src/shm_ring.hpp's static_asserts.
