//! Live integration test against a *loaded* ScreenExtend Audio driver (macOS 10.15–12.x). Exercises
//! the real shipped Rust: probe → routing.activate (takes over default output) → shm_reader (the
//! actual `ShmMap`/reader thread, or the HAL-input fallback) → playthrough → volume proxy → control
//! thread → stop/restore. Ignored by default (it takes over the system default output and needs the
//! driver installed); run explicitly on a box with the driver loaded:
//!
...
fn legacy_live_capture_roundtrip() {
    ...
    // The reader thread should have pushed non-silent captured samples into the encoder ring.
