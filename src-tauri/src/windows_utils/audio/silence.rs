//! Silent render companion (PRD §4.3).
//!
//! The §3.2 spike proved this empirically: with nothing playing, the Windows audio engine
//! parks the render endpoint and loopback capture delivers *no* packets and its event never
//! fires (0/20 signals idle). Opening a second, silent render stream on the same endpoint
//! keeps the engine running, which makes loopback produce continuous silence packets *and*
//! makes the capture event fire (20/20, ~9.4 ms). So this is the clock source, not merely an
//! idle-silence patch.
//!
//! Started lazily by the capture thread (only while audio is enabled) and stopped when capture
//! tears down, so the host's audio engine is never pinned awake for a feature nobody is using.
//! Uses `AUDCLNT_BUFFERFLAGS_SILENT` on render so the engine treats the buffer as silence
//! without us writing zeros — inaudible, and cheap.
...
impl SilenceCompanion {
    /// Start the companion. Blocks until the render stream is running (or errors during setup).
...
fn run(stop: Arc<AtomicBool>, ready_tx: std::sync::mpsc::Sender<Result<()>>) {
    // SAFETY: all WASAPI/COM calls below run on this dedicated MTA thread; handles are
    // released by RAII guards (`ComGuard`, `EventHandle`) on any exit path.
...
unsafe fn setup(
    ...
    // Pre-roll one buffer of silence before starting, so the engine has data immediately.
    let _buf = render
...
unsafe fn pump(
    ...
    while !stop.load(Ordering::Relaxed) {
        // Wake each period; 100 ms timeout bounds stop latency if the event stalls.
        event.wait(100);
