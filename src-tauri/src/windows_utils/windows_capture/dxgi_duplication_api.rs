...
//! Windows DXGI Desktop Duplication API. It integrates with [`super::monitor::Monitor`]
//! to select the target output and exposes CPU-readable frames via [`super::frame::FrameBuffer`].
//!
//! # Example
//! ```no_run
...
use super::d3d11::{
    create_d3d_device, unmap_staging_texture, MappedStagingTexture, StagingTexture,
};
use super::encoder::{ImageEncoder, ImageEncoderError, ImageEncoderPixelFormat, ImageFormat};
...
pub enum Error {
    ...
    DirectXError(#[from] super::d3d11::Error),
    ...
    ///
    /// Wraps [`super::encoder::ImageEncoderError`].
    #[error("Failed to encode the image buffer to image bytes with the specified format: {0}")]
    ImageEncoderError(#[from] super::encoder::ImageEncoderError),
...
/// It integrates with [`super::monitor::Monitor`] to select the target output.
...
impl DxgiDuplicationApi {
    ...
    pub fn new(monitor: Monitor) -> Result<Self, Error> {
        ...
        let duplication =
            unsafe { output.DuplicateOutput1(&d3d_device, 0, &DEFAULT_DUPLICATION_FORMATS)? };
    ...
    pub fn recreate(mut self) -> Result<Self, Error> {
        ...
        self.duplication = unsafe {
            self.output
                .DuplicateOutput1(&self.d3d_device, 0, &DEFAULT_DUPLICATION_FORMATS)?
        };
    ...
    pub fn recreate_options(
        ...
        self.duplication = unsafe {
            self.output
                .DuplicateOutput1(&self.d3d_device, 0, &supported_formats)?
        };
        self.duplication_desc = unsafe { self.duplication.GetDesc() };
        self.is_holding_frame = false;
...
/// Call [`DxgiDuplicationFrame::buffer`] to obtain a CPU-readable [`super::frame::FrameBuffer`].
...
impl<'a> DxgiDuplicationFrame<'a> {
    ...
    /// you can use [`super::frame::FrameBuffer::as_nopadding_buffer`] to obtain a packed
    /// representation.
    #[inline]
    ...
    /// Advanced: reuse your own CPU staging texture ([`super::d3d11::StagingTexture`]).
    ///
