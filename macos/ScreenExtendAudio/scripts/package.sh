#!/usr/bin/env bash
#
# Package ScreenExtendAudio.driver into a signed installer .pkg (PRD-macos-legacy-audio.md §8.4).
#
# The .pkg installs the bundle to /Library/Audio/Plug-Ins/HAL (root-owned) and runs scripts/
# postinstall to chown it and restart coreaudiod. Installing there requires admin rights — there is
# no fully silent install, and that is by design.
#
# Usage:
#   ./scripts/build.sh                                   # build the .driver first
#   INSTALLER_ID="Developer ID Installer: …" ./scripts/package.sh
#   ./scripts/package.sh                                 # unsigned (dev only; will not load on
#                                                        # modern macOS without notarization)
set -euo pipefail
...
# Wrap in a product archive so it can be notarized and (optionally) installer-signed.
